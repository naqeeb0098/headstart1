	...
	def on_submit(self):
		try:
			for obj in self.student_details:
				...
				structure = frappe.get_doc('Student Structure Allocation',{'student':obj.student,'docstatus':1,'custom_enabled':1},['fee_structure','fee_structure_components'])

				struct = frappe.get_doc('Fee Structure',{'name':structure.fee_structure})
				fee_structure_components = struct.components
				...
				components_list = frappe.db.get_all('Fee Component',{'parent':structure.fee_structure},['fees_category','amount'])
				total_discount = 0
				for p in policies:
					if p['discount']:
				...
				fees = {'doctype':'Fees',
						...
						'components': fee_structure_components,
						...
						'custom_total_fee':float(structure.total_amount),
